...
function Course(props) {
  ...
  course.link = courseData.data.url;
  course.price = courseData.data.price.replace("$", "");
  course.headline = courseData.data.headline;
  course.title = courseData.data.title;
  ...
  return (
    ...
    >
      <img
        src={courseData.data.image_480x270}
        alt={courseData.data.title}
        style={{ width: "100px", marginRight: "20px" }}
      />
      <List.Item.Meta
        title={`${courseData.data.title} | ID: ${course.idCourse}`}
        description={courseData.data.headline}
      />
    </List.Item>
  );
}